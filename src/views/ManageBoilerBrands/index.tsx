...
import { getBrand, addBrand, deleteBrand, deleteModel } from '../../redux/actions'
...
interface State {
  /* showCreateCmsModal: boolean; */
  currentRowData: { [key: string]: any } | null
}

const initialState: State = {
  /* showCreateCmsModal: false, */
  currentRowData: null,
}

const ManageBoilerBrands: React.FC<LoginProps> = function ({ dispatch }) {
  ...
  const handleEditBrand = (data: any) => {
    setVisibleAddBrand(true), setVisibleEditBrand(true), setBrandDetails(data)
      ...
      render: (text, row: any) => (
        <Col span={24}>
          <Button
            ...
            // onClick={()=>{setVisibleAddBrand(true),setVisibleEditBrand(true),setBrandDetails(row)}}
  ...
  const statusChanged = (checked: boolean, row: any) => {
    const brandId = row._id
    const status = checked ? 'active' : 'deactive'
    const payload = { brandId, status }
    setCurrentActiveId(brandId)
    dispatch(addBrand(payload, 'edit')).then(res => {
      if (res.data) {
  ...
  function getCMSTableData(params: any) {
    return dispatch(getBrand(params)).then(res => {
  ...
  function handleActionButton(e, brandId) {
    if (!brandId) {
      notification.error({
        message: `Error`,
    ...
    setCurrentDeleteActiveId(brandId)
    dispatch(deleteBrand(brandId)).then(res => {
          ...
          >
            <Row justify="center">
              ...
              >
                <Table
                  allowAdd={true}
                  tableTitle="Boiler Brands"
                  moduleName="brand"
                  ref={tableRef}
                  getTableData={getCMSTableData}
                  columns={columns}
                  onDelete={deleteBrand}
                  onAdd={() => setVisibleAddBrand(true)}
                />
                <CreateContent
                  ...
                  onCancel={() => (
                    setVisibleAddBrand(false), setVisibleEditBrand(false), setBrandDetails('')
                  )}
...
export default connect()(ManageBoilerBrands)
