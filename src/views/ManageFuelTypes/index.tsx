...
import { getFuelType, addFuelType, deleteFuelType } from '../../redux/actions'
...
const ManageFuelTypes: React.FC<LoginProps> = function ({ dispatch }) {
  ...
  const statusChanged = (checked: boolean, row: any) => {
    const fuelTypeId = row._id
    const status = checked ? 'active' : 'deactive'
    const payload = { fuelTypeId, status }
    setCurrentActiveId(fuelTypeId)
    dispatch(addFuelType(payload, 'edit')).then(res => {
      if (res.data) {
  ...
  function getCMSTableData(params: any) {
    return dispatch(getFuelType(params)).then(res => {
  ...
  function handleActionButton(e, fuelTypeId) {
    if (!fuelTypeId) {
      notification.error({
        message: `Error`,
    ...
    setCurrentDeleteActiveId(fuelTypeId)
    dispatch(deleteFuelType(fuelTypeId)).then(res => {
...
export default connect()(ManageFuelTypes)
