...
import { getBoilerModel, addBoilerModel, deleteModel } from '../../redux/actions'
...
const ManageBoilerModels: React.FC<LoginProps> = function ({ dispatch }) {
  ...
  const columns: any = [
    {
      ...
      ...getColumnSearchProps('boilerBrandId'),
      render: (name, row) => <span>{name ? name.name : ''}</span>,
    },
    {
      title: 'Name',
      dataIndex: 'name',
  ...
  const statusChanged = (checked: boolean, row: any) => {
    const modelId = row._id
    const status = checked ? 'active' : 'deactive'
    const payload = { modelId, status }
    setCurrentActiveId(modelId)
    dispatch(addBoilerModel(payload, 'edit')).then(res => {
      if (res.data) {
  ...
  function getCMSTableData(params: any) {
    return dispatch(getBoilerModel(params)).then(res => {
  ...
  function handleActionButton(e, modelId) {
    if (!modelId) {
      notification.error({
        message: `Error`,
    ...
    setCurrentDeleteActiveId(modelId)
    dispatch(deleteModel(modelId)).then(res => {
...
export default connect()(ManageBoilerModels)
