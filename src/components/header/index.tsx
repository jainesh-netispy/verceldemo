...
const HomeHeader: React.FC<Props> = function ({ collapsed, setCollapsed, userInfo }) {
    ...
    () => (
      <div className="message-popover">
        <div className="msg-header item-block">
          ...
          <Link className="right" to={SETTING.NOTIFICATION.path}>
            消息接收管理
          </Link>
        </div>
        {messageList.length > 0 ? (
          <>
            ...
            <Link className="item-block ls" to={SETTING.INNER_MESSAGE.path}>
              查看更多
            </Link>
          </>
        ) : (
